...
describe("applyToJob", function () {
  test("works", async function () {
    ...
    const res = await db.query("SELECT * FROM applications WHERE username = 'u1' AND job_id = 1");
    expect(res.rows.length).toEqual(1);
  });

  test("not found if user does not exist", async function () {
    try {
      await User.applyToJob("no-user", 1);
      fail();
    } catch (err) {
      expect(err instanceof NotFoundError).toBeTruthy();
    }
...
describe("get with jobs", function () {
  test("works", async function () {
    await User.applyToJob("u1", 1);
    const user = await User.get("u1");
    expect(user.jobs).toEqual([1]);
  });
});
