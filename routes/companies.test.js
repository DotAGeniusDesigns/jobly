...
describe("GET /companies with filters", function () {
  ...
  test("fails: invalid filter", async function () {
    const resp = await request(app).get("/companies").query({ invalid: "filter" });
    expect(resp.statusCode).toEqual(400);
  });
...
describe("GET /companies/:handle", function () {
  test("works for anon: company with jobs", async function () {
    const resp = await request(app).get("/companies/c1");
    expect(resp.body.company.jobs).toEqual([
      { id: expect.any(Number), title: "Software Engineer", salary: 120000, equity: "0.05" },
      { id: expect.any(Number), title: "Data Scientist", salary: 110000, equity: "0" },
    ]);
  });
});
